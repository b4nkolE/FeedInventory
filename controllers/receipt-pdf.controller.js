...
export const downloadReceipt = async (req, res) => {
    try {
        ...
        if (isThermal) {
            // Calculate height dynamically based on item count, name wrapping, buyer info, and notes
            let dynamicHeight = 260; // base padding and header/footer space
            for (const item of sale.items) {
                const nameLength = item.feedItem.name.length;
                const nameLines = Math.ceil(nameLength / 20); // ~20 chars per line in 95pt column
                dynamicHeight += nameLines * 10 + 12;
            }
        ...
        if (isThermal) {
            ...
            if (reference) {
                ...
                doc.font("Helvetica").text(reference, 62, currentY);
                currentY += 10;
            }
            ...
            for (const item of sale.items) {
                doc.text(item.feedItem.name, 12, currentY, { width: 95 });
                doc.text(String(item.quantity), 110, currentY, { width: 25, align: "right" });
                doc.text(
                ...
                const itemHeight = doc.heightOfString(item.feedItem.name, { width: 95 });
            ...
            doc.fontSize(11)
                .fill(TEXT)
                .font("Helvetica")
                .text(reference || "N/A", 350, metaY + 14);
            ...
            sale.items.forEach((item, index) => {
                ...
                doc.text(item.feedItem.name, cols.item + 8, rowY + 2, { width: 160 });
                doc.text(item.feedItem.category?.name || "N/A", cols.category + 8, rowY + 2, { width: 90 });
                doc.text(String(item.quantity), cols.qty + 8, rowY + 2, { width: 60 });
                doc.text(
                ...
                doc.text(
                    `₦${Number(item.totalPrice).toLocaleString("en-NG", { minimumFractionDigits: 2 })}`,
                    cols.total + 8,
                    rowY + 2,
                    { width: 80 }
                );
